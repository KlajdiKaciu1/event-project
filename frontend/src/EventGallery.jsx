      ...
      <div className="fixed inset-0 bg-black text-white min-h-screen z-50 overflow-y-auto">
        <div className="bg-black p-8 grid gap-4">
          <div className="flex justify-between items-center">
            <h2 className="text-3xl md:text-4xl lg:text-5xl">Photos of {event.title}</h2>
            <button
        ...
        <div className="aspect-square overflow-hidden">
          {event.photos?.[0] && (
            <img
              ...
              src={"http://localhost:4000/" + event.photos[0]}
              alt=""
            />
          )}
        </div>
          ...
          {event.photos?.[1] && (
            <div className="aspect-square overflow-hidden">
              <img
                onClick={() => setShowAllPhotos(true)}
                className="cursor-pointer object-cover w-full h-full"
                src={"http://localhost:4000/" + event.photos[1]}
                alt=""
              />
            </div>
          )}
          {event.photos?.[2] && (
            <div className="aspect-square overflow-hidden">
              <img
                onClick={() => setShowAllPhotos(true)}
                className="cursor-pointer object-cover w-full h-full"
                src={"http://localhost:4000/" + event.photos[2]}
                alt=""
              />
            </div>
          )}
